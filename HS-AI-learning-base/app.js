...
const roomAvailability = document.querySelector('#roomAvailability');
...
window.addEventListener('keydown', (event) => {
  ...
  if (event.key === 'Enter' && !event.repeat && !isTextEntry) {
    if (roomDialog.open) {
      ...
      if (!enterRoom.disabled) enterRoom.click();
...
enterButton.addEventListener('click', () => {
  ...
  roomAvailability.textContent = dialogRoom.available
    ? '確認後將進入獨立的房間挑戰。'
    : '本房內容正在製作中，目前先開放共通地圖與入口位置測試。';
  enterRoom.disabled = !dialogRoom.available;
  enterRoom.textContent = dialogRoom.available ? '進入房間' : '房間製作中';
  roomDialog.showModal();
  (dialogRoom.available ? enterRoom : document.querySelector('#closeDialog')).focus();
});
...
enterRoom.addEventListener('click', () => {
  ...
  if (!dialogRoom.available) return;
  const roomUrl = new URL(dialogRoom.folder, window.location.href);
  const entryId = new URLSearchParams(window.location.search).get('entry');
