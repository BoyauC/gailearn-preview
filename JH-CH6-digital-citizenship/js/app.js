...
function statusBar() {
  return `<section class="statusbar" aria-label="角色狀態">${Object.entries(STATES).map(([key, meta]) => {
    ...
    return `<div class="status ${meta.risk ? "risk" : ""}"><div class="status-head"><span>${meta.icon} ${meta.name}</span><span>${shown}</span></div><div class="meter" aria-label="${meta.name} ${shown}"><span style="--value:${shown}%"></span></div></div>`;
  }).join("")}</section>`;
}

function renderHome() {
  const resume = state.started && !state.completed;
  app.innerHTML = shell(`<section class="hero"><div class="hero-art" role="img" aria-label="小行，數位生活挑戰營的國中生主角"></div><div><div class="eyebrow">國中版・CH6・六天生活旅程</div><h2>每一次點擊，<br>都會留下足跡。</h2><p class="hero-copy">和小行、吱吱、思思、可可與歐匿一起面對群組消息、AI 作業、照片、個資、網路素材與匿名爭論。沒有只靠直覺的滿分答案；你要在時間、關係與風險之間做決定。</p><div class="chips"><span class="chip">6 項數位公民能力</span><span class="chip">4 項生活狀態</span><span class="chip">延遲後果與補救</span></div><button class="primary" id="start">${resume ? "繼續挑戰" : state.completed ? "查看上次結算" : "開始六天挑戰"}</button></div></section>`);
...
const CONSEQUENCES = [
  ...
  { id: "late_d3", day: 3, flag: "late_night_overuse", text: "昨晚睡太晚，小行今天比較難集中精神。", states: { ST_ENERGY: -8 } },
  { id: "photo_d5", day: 5, flag: "photo_forwarded", text: "阿岳的迷因仍有副本，出現在班級影片草稿中。", states: { ST_TRUST: -4, ST_FOOTPRINT: 5 } },
  { id: "source_d4", day: 4, flag: "source_checked", text: "思思記得小行會查來源，主動請你一起檢查活動頁。", states: { ST_TRUST: 4 } },
...
function renderMap() {
  ...
  app.innerHTML = shell(`${statusBar()}<section class="map-card"><div class="map-head"><div><div class="eyebrow">六天生活地圖</div><h2>下一站：第 ${state.day} 天</h2></div><div class="map-avatar" aria-label="小行目前在第 ${state.day} 天節點"></div></div>${pendingNotice ? `<div class="consequence"><strong>延遲後果：</strong>${pendingNotice}</div>` : ""}<div class="road">${content.days.map((day) => {
    const done = state.completedDays.includes(day.day);
    const current = day.day === state.day;
    const locked = day.day > state.day;
    return `<button class="node ${done ? "done" : ""} ${current ? "current" : ""}" data-day="${day.day}" ${locked ? "disabled" : ""}><span class="badge-shell"><img src="${badgePath(day.axis)}" alt="${AXES[day.axis].name}"></span><span class="node-label">第 ${day.day} 天<br>${day.title}</span><span class="node-mark">${done ? "✓ 已完成" : current ? "今日任務" : "回顧"}</span></button>`;
  }).join("")}</div><button class="primary" id="enter">開始第 ${state.day} 天任務</button></section>`, controls);
  document.querySelector("#enter").addEventListener("click", () => startDay(state.day));
...
const ENDINGS = {
  ...
  END_FOOTPRINT_ALERT: ["數位足跡警報", "有些資料、照片或留言仍需處理；這是補救任務，不是單純失敗。"],
