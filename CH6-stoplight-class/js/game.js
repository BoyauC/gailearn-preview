/* =========================================================
   ...
   - 班級版：基礎關結束與遊戲完成時，經由 StoplightClassBridge 批次送出
   - 教師密碼不會由本檔案讀取或保存
       * 所有錯誤回饋為鼓勵式、非羞辱式
   ========================================================= */
(function () {
  ...
  function ingestRows(rows) {
    ...
    rows.forEach((r) => {
      ...
      const item = {
        id: r.id || r.questionId || r.item_id || '',
        signal: signal,
  ...
  function showScreen(screenEl) {
    [el.screenJoin, el.screenStart, el.screenGame, el.screenTransition, el.screenResult]
      .filter(Boolean)
  ...
  /* =========================================================
     ...
     ========================================================= */
  function startBasic() {
    const bridge = window.StoplightClassBridge;
    if (bridge && typeof bridge.beginAttempt === 'function') bridge.beginAttempt();
    state.level = 'basic';
  ...
  function recordAttempt(choice, timedOut) {
    const q = state.queue[state.index];
    if (!q) return;
    state.attemptLog.push({
      ...
      elapsedMs: Math.max(0, Math.round(performance.now() - state.questionStartedAt))
    });
  }
  ...
  function finishGame() {
    ...
    if (!state.finalSaved) {
      ...
      saveClassCheckpoint('final');
    }
  }

  function saveClassCheckpoint(phase) {
    const bridge = window.StoplightClassBridge;
    if (!bridge || typeof bridge.saveCheckpoint !== 'function') return Promise.resolve();
    const snapshot = state.attemptLog.map((attempt) => ({ ...attempt }));
    return bridge.saveCheckpoint(phase, snapshot).catch(() => {
      // student.js 會顯示可操作的重試按鈕；此處不輸出可能包含作答內容的日誌。
    });
  }
  ...
  function renderTransition() {
    ...
    if (c >= PASS_THRESHOLD) {
      el.transitionMsg.innerHTML =
        `你答對了 <strong>${c}</strong> 題，很棒的思考！<br />準備好挑戰限時 10 秒的進階題了嗎？`;
  ...
  function bindEvents() {
    ...
    if (el.btnStart) el.btnStart.addEventListener('pointerdown', onStartClick);
    if (el.btnGoAdvanced) el.btnGoAdvanced.addEventListener('pointerdown', () => startAdvanced());
    if (el.btnFinishEarly) el.btnFinishEarly.addEventListener('pointerdown', finishGame);
    if (el.btnRestart) el.btnRestart.addEventListener('pointerdown', () => startBasic());
  ...
  async function boot() {
    bindEvents();
    if (el.body.dataset.classMode === 'student') {
      const initializeClassGame = (bridge) => {
        if (!bridge || !Array.isArray(bridge.questions)) {
          showLoadError('課堂題組格式不正確');
          return;
        }
        ingestRows(bridge.questions);
        showScreen(el.screenStart);
      };

      if (window.StoplightClassBridge) {
        initializeClassGame(window.StoplightClassBridge);
      } else {
        window.addEventListener('stoplight:session-ready', (event) => initializeClassGame(event.detail), { once: true });
      }
      return;
    }
    try {
